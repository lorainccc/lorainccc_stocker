	...
	<div id="primary" class="content-area">
		...
		<?php while ( have_posts() ) : the_post(); ?>
			...
			$url = 'https://ticketing.lorainccc.edu/stockerartscenter/website/';
				...
				switch($iframeurl){
					case "ChooseSeats":
						echo '    <iframe style="width: 100%; border:0;" src="' . $url . 'ChooseSeats.aspx?' . $style . '&resize=true&EventInstanceId=' . $lc_event_id .'" onload="setTimeout(function(){ window.scrollTo(0,0);}, 100)" name="SpektrixIFrame" id="SpektrixIFrame" title="LCCC Stocker Art Center\'s Choose Seats page in Spektrix"></iframe>';
						break;
					case "Basket":
						echo '    <iframe style="width: 100%; border:0;" src="' . $url . 'Basket2.aspx?' . $style . '&resize=true" onload="setTimeout(function(){ window.scrollTo(0,0);}, 100)" name="SpektrixIFrame" id="SpektrixIFrame" title="LCCC Stocker Art Center\'s Shopping Basket page in Spektrix"></iframe>';
						break;
					case "MyAccount":
						echo '    <iframe style="width: 100%; border:0;" src="' . $url . 'Secure/MyAccount.aspx?' . $style . '&resize=true" onload="setTimeout(function(){ window.scrollTo(0,0);}, 100)" name="SpektrixIFrame" id="SpektrixIFrame" title="LCCC Stocker Art Center\'s MyAccount page in Spektrix"></iframe>';
						break;
					case "Checkout":
